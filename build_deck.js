...
const W=13.333, H=7.5, M=0.8, A="assets/";   // 0.8in = 2.03cm = IGC house left margin
...
function tabEyebrow(s,text){
  ...
  s.addText(text,{x:M,y:0.36,w,h:0.36,fontFace:SANS_M,fontSize:9,color:ORANGE_D,align:"center",valign:"middle",margin:0});
}
function chrome(s,{eyebrow,title,subtitle,num,source,page,band=false}={}){
  ...
  if(title) s.addText(title,{x:tx,y:0.8,w:W-tx-1.5,h:0.6,fontFace:SANS_SB,fontSize:18,color:GRAPH,align:"left",valign:"middle",margin:0});
  if(subtitle) s.addText(subtitle,{x:M,y:1.42,w:W-M-1.5,h:0.48,fontFace:SANS,fontSize:10.5,color:GRAY,align:"left",valign:"top",margin:0,lineSpacingMultiple:1.02});
  if(source) s.addText(source,{x:M,y:band?H-1.06:H-0.6,w:W-2.0,h:0.24,fontFace:SANS,fontSize:7,color:GRAY_L,align:"left",valign:"middle",margin:0});
  if(!band){
...
function conclusionBand(s,runs,{y=H-0.75,h=0.75,page=null}={}){
  rect(s,0,y,W,h,GRAPH);
  rect(s,0,y,0.14,h,ORANGE);
  s.addText(runs.map(r=>({text:r[0],options:{color:r[1]?ORANGE:"E8ECF1"}})),
    {x:2.0,y,w:W-4.0,h,fontFace:SANS_SB,fontSize:12,align:"center",valign:"middle",margin:0,lineSpacingMultiple:1.0});
  if(page!=null){ // the band doubles as the footer strip — igc mark reversed out in white
    s.addImage({path:A+"igc_white.png",x:M,y:y+h/2-0.145,w:0.32,h:0.32*IGC_AR});
    s.addText([{text:String(page),options:{color:WHITE,fontFace:SANS_M}},{text:"    Confidential",options:{color:MUTE_ON_DARK,fontFace:SANS}}],
      {x:M+0.42,y:y+h/2-0.15,w:3,h:0.3,fontSize:9,align:"left",valign:"middle",margin:0});
  }
}
function colChart(s,type,data,x,y,w,h,opts={}){
  s.addChart(type,data,Object.assign({
    ...
    catAxisLabelFontFace:SANS,catAxisLabelFontSize:9,catAxisLabelColor:GRAY,catGridLine:{style:"none"},catAxisLineShow:false,
...
(()=>{ const s=p.addSlide(); bg(s,GRAPH);
  ...
  s.addText("PROJECT COMPASS",{x:cut+0.77,y:3.25,w:3.55,h:0.9,fontFace:SANS_SB,fontSize:18,color:WHITE,align:"center",valign:"middle",charSpacing:2,margin:0});
  ...
  s.addText("São Paulo, 2026  |  Confidencial",{x:cut+0.77,y:4.72,w:3.55,h:0.3,fontFace:SANS_M,fontSize:9,color:ORANGE,align:"center",margin:0});
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  s.addText(disc.map(t=>({text:t,options:{breakLine:true,paraSpaceAfter:8}})),{x:M,y:1.62,w:5.85,h:5.2,fontFace:SANS,fontSize:8,color:TXT,align:"justify",valign:"top",lineSpacingMultiple:1.04});
  s.addText(disc2.map(t=>({text:t,options:{breakLine:true,paraSpaceAfter:8}})),{x:6.7,y:1.62,w:6.05,h:3.4,fontFace:SANS,fontSize:8,color:TXT,align:"justify",valign:"top",lineSpacingMultiple:1.04});
  ...
  s.addText("Bruno Iervolino",{x:7.05,y:5.35,w:4,h:0.35,fontFace:SANS_SB,fontSize:14,color:WHITE,margin:0});
...
function tocSlide(activeIdx){
  ...
  // headline
  s.addText("Executive",{x:M,y:1.0,w:6,h:0.75,fontFace:SANS_SB,fontSize:32,color:WHITE,margin:0});
  s.addText("Summary",{x:M,y:1.72,w:6,h:0.75,fontFace:SANS_SB,fontSize:32,color:ORANGE,margin:0});
  ...
  SECTIONS.forEach((sec,i)=>{
    ...
    s.addText(sec[1],{x:M+5.2,y:y,w:0.8,h:0.5,fontFace:SANS_M,fontSize:12,color:active?WHITE:MUTE_ON_DARK,valign:"middle",margin:0});
    y+=0.78;
  });
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  rows.forEach(r=>{
    ...
    s.addText(r[1],{x:M+1.35,y:ry-0.02,w:pw-1.7,h:0.3,fontFace:SANS_SB,fontSize:11,color:WHITE,valign:"top",margin:0});
    s.addText(r[2],{x:M+1.35,y:ry+0.28,w:pw-1.7,h:0.62,fontFace:SANS,fontSize:8,color:"AEB6C2",valign:"top",margin:0,lineSpacingMultiple:1.0});
    ry+=1.12;
  });
  ...
  tiles.forEach((t,i)=>{ const x=rx+(i%2)*(tw+0.3), y=2.38+Math.floor(i/2)*(th+0.16);
    ...
    s.addText(t[1],{x:x+0.2,y:y+0.6,w:tw-0.36,h:0.32,fontFace:SANS,fontSize:8,color:GRAY,margin:0,valign:"top"});
  });
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  s.addText("Gross revenue by entity — 1Q25 vs. 1Q26",{x:rx+0.3,y:2.22,w:rw-1.7,h:0.3,fontFace:SANS_SB,fontSize:11,color:GRAPH,margin:0});
  s.addText("BRL Mn",{x:rx+0.3,y:2.5,w:2,h:0.24,fontFace:SANS,fontSize:8,color:GRAY,margin:0});
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  s.addText([{text:"83%",options:{fontFace:SANS_SB,color:ORANGE}},{text:" of 1Q26",options:{fontFace:SANS,color:GRAY,fontSize:8}}],{x:M+cw-1.55,y:2.36,w:1.3,h:0.44,fontSize:12,align:"center",valign:"middle",margin:0});
  const lpts=[
  ...
  let ly=3.1; lpts.forEach(t=>{ arrow(s,M+0.34,ly+0.06,0.16);
    s.addText(t,{x:M+0.62,y:ly-0.04,w:cw-0.95,h:0.68,fontFace:SANS,fontSize:9,color:TXT,valign:"top",margin:0,lineSpacingMultiple:1.0}); ly+=0.72; });
  ...
  s.addText([{text:"17%",options:{fontFace:SANS_SB,color:ORANGE}},{text:" of 1Q26",options:{fontFace:SANS,color:GRAY,fontSize:8}}],{x:M+2*cw-1.05,y:2.36,w:1.3,h:0.44,fontSize:12,align:"center",valign:"middle",margin:0});
  const rpts=[
  ...
  ly=3.1; rpts.forEach(t=>{ arrow(s,M+cw+0.84,ly+0.06,0.16);
    s.addText(t,{x:M+cw+1.12,y:ly-0.04,w:cw-0.95,h:0.68,fontFace:SANS,fontSize:9,color:TXT,valign:"top",margin:0,lineSpacingMultiple:1.0}); ly+=0.72; });
  // entity stat bands (Alquimia bottom bands)
  const by=5.55, bh=0.85;
  rrect(s,M,by,cw,bh,GRAPH,{radius:0.1});
  s.addText([{text:"AGM Caetano   ",options:{fontFace:SANS_SB,fontSize:12,color:WHITE}},{text:"BRL 36.2 Mn",options:{fontFace:SANS_SB,fontSize:14,color:ORANGE}},{text:"  1Q26 gross revenue   ·   ",options:{fontFace:SANS,fontSize:9,color:"AEB6C2"}},{text:"+36% YoY",options:{fontFace:SANS_SB,fontSize:11,color:WHITE}}],
    {x:M+0.25,y:by,w:cw-0.5,h:bh,align:"center",valign:"middle",margin:0});
  rrect(s,M+cw+0.5,by,cw,bh,ORANGE,{radius:0.1});
  s.addText([{text:"AGM Alpha   ",options:{fontFace:SANS_SB,fontSize:12,color:WHITE}},{text:"BRL 7.3 Mn",options:{fontFace:SANS_SB,fontSize:14,color:GRAPH}},{text:"  1Q26 gross revenue   ·   ",options:{fontFace:SANS,fontSize:9,color:"FBD9C6"}},{text:"+74% YoY",options:{fontFace:SANS_SB,fontSize:11,color:WHITE}}],
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  pil.forEach((t,i)=>{
    ...
    s.addText(String(i+1),{x:M,y,w:d,h:d,fontFace:SANS_SB,fontSize:20,color:WHITE,align:"center",valign:"middle",margin:0});
    s.addText([{text:t[0],options:{fontFace:SANS_SB,color:GRAPH}},{text:"  —  "+t[1],options:{fontFace:SANS,color:GRAY}}],
      {x:M+0.85,y:y-0.05,w:7.35,h:0.75,fontSize:11,valign:"middle",margin:0,lineSpacingMultiple:1.02});
    y+=0.92;
  });
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  chrome(s,{eyebrow:"Investment Thesis",num:1,title:"A solid track record with blue-chip clients",
    ...
    source:"Source: Company. Client relationship metrics (contracts delivered, years, share of revenue) to be confirmed with management.",page:9,band:true});
  s.addText("SELECTED CLIENTS",{x:M,y:2.05,w:5,h:0.26,fontFace:SANS_SB,fontSize:9,color:ORANGE,charSpacing:1.5,margin:0});
  ...
  s.addText("PORTFOLIO CHARACTERISTICS",{x:rx,y:2.05,w:rw,h:0.26,fontFace:SANS_SB,fontSize:9,color:ORANGE,charSpacing:1,margin:0});
  ...
  stats.forEach(t=>{ rrect(s,rx,y,rw,0.8,PAPER,{radius:0.09,line:LINE}); iconChip(s,rx+0.18,y+0.15,0.5,t[0],{fill:WHITE});
    s.addText(t[1],{x:rx+0.82,y:y+0.1,w:rw-1,h:0.38,fontFace:SANS_SB,fontSize:16,color:GRAPH,margin:0,valign:"middle"});
    s.addText(t[2],{x:rx+0.82,y:y+0.46,w:rw-1,h:0.28,fontFace:SANS,fontSize:9,color:GRAY,margin:0,valign:"top"});
    y+=0.93; });
  conclusionBand(s,[["A recurring, contracted revenue base with ",0],["blue-chip clients",1],[" underpins visibility and resilience",0]],{page:9});
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  chrome(s,{eyebrow:"Investment Thesis",num:2,title:"A modern, fully-owned fleet tailored to client needs",
    ...
    source:"Source: Company; Jan-2026 fleet register. (1) Average age per Company materials. (2) 373 owned vehicles per Jan-26 fleet register; +400 aggregate fleet per Company materials. (3) Value of the fleet per Company materials.",page:10,band:true});
  ...
  let hy=2.67; hl.forEach((t,i)=>{ s.addText(t[0],{x:px+0.34,y:hy,w:pw-0.6,h:0.44,fontFace:SANS_SB,fontSize:20,color:WHITE,margin:0,valign:"bottom"});
    s.addText(t[1],{x:px+0.34,y:hy+0.46,w:pw-0.6,h:0.3,fontFace:SANS,fontSize:9,color:"AEB6C2",margin:0,valign:"top"});
  ...
  s.addText("LEADING CHASSIS & EQUIPMENT BRANDS",{x:rx,y:4.2,w:rw,h:0.26,fontFace:SANS_SB,fontSize:8,color:ORANGE,charSpacing:0.5,margin:0});
  ...
  conclusionBand(s,[["A ",0],["fully-owned, modern fleet",1],[" creates operational flexibility and barriers that are hard to replicate",0]],{page:10});
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  chrome(s,{eyebrow:"Investment Thesis",num:3,title:"Technology-enabled operations support high reliability",
    ...
    source:"Source: Company. Availability and preventive-maintenance metrics, and scope of in-house maintenance, to be confirmed with management.",page:11,band:true});
  const groups=[
  ...
  groups.forEach((g,i)=>{ const x=M+(i%2)*(gw+0.4), y=2.05+Math.floor(i/2)*(gh+0.25);
    ...
    s.addText(g[1],{x:x+0.86,y:y+0.22,w:gw-1.05,h:0.44,fontFace:SANS_SB,fontSize:11,color:GRAPH,valign:"middle",margin:0,lineSpacingMultiple:0.95});
    s.addText(g[3],{x:x+0.24,y:y+0.76,w:gw-0.48,h:0.5,fontFace:SANS,fontSize:8,color:GRAY,valign:"top",margin:0,lineSpacingMultiple:1.0});
  ...
  conclusionBand(s,[["Intensive ",0],["monitoring and telemetry",1],[" ensure a high level of service across the network",0]],{page:11});
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  chrome(s,{eyebrow:"Investment Thesis",num:4,title:"Multiple avenues for continued value creation",
    ...
    source:"Source: Company; press research. Growth avenues to be validated with management.",page:12,band:true});
  const av=[
  ...
  av.forEach((a,i)=>{ const x=M+i*(cw+0.4), y=2.3, h=3.55;
    ...
    s.addText(String.fromCharCode(65+i),{x:x+cw-0.7,y:y+0.25,w:0.5,h:0.5,fontFace:SANS_SB,fontSize:20,color:O_TINT2,align:"center",valign:"middle",margin:0});
    s.addText(a[1],{x:x+0.3,y:y+1.4,w:cw-0.6,h:0.75,fontFace:SANS_SB,fontSize:14,color:GRAPH,align:"center",valign:"top",margin:0,lineSpacingMultiple:1.0});
    s.addText(a[2],{x:x+0.32,y:y+2.2,w:cw-0.64,h:1.2,fontFace:SANS,fontSize:9,color:GRAY,align:"center",valign:"top",margin:0,lineSpacingMultiple:1.05});
  });
  conclusionBand(s,[["A clear and ",0],["executable path",1],[" for continued value creation",0]],{page:12});
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  chrome(s,{eyebrow:"Company Overview",title:"A track record built over more than 20 years",
    ...
    source:"Source: Company. Foundation year and milestone dates to be confirmed with management.",page:14,band:true});
  ...
  nodes.forEach((nd,i)=>{ const x=M+i*(cw+gap); const last=i===n-1;
    ...
    s.addText(nd[0],{x:x+0.22,y:cy+0.25,w:cw-0.44,h:0.55,fontFace:SANS_SB,fontSize:20,color:WHITE,margin:0,valign:"middle"});
    ...
    s.addText(nd[1].map((m,j)=>({text:m,options:{breakLine:true,paraSpaceAfter:6}})),
      {x:x+0.22,y:cy+1.1,w:cw-0.44,h:chh-1.3,fontFace:SANS,fontSize:9,color:last?"C7CDD6":WHITE,valign:"top",margin:0,lineSpacingMultiple:1.05});
    if(i<n-1) arrow(s,x+cw+gap/2-0.08,cy+chh/2-0.08,0.16,GRAY_L);
  });
  conclusionBand(s,[["More than ",0],["two decades",1],[" of disciplined operation, positioned for its next phase of growth",0]],{page:14});
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  s.addText("Headquarters — Goiânia, GO",{x:rx+1.02,y:2.14,w:rw-1.25,h:0.32,fontFace:SANS_SB,fontSize:14,color:WHITE,valign:"top",margin:0});
  s.addText("Corporate HQ (Garagem Central) centralizing fleet management, monitoring infrastructure and administration",{x:rx+1.02,y:2.46,w:rw-1.25,h:0.5,fontFace:SANS,fontSize:9,color:"AEB6C2",valign:"top",margin:0,lineSpacingMultiple:1.0});
  s.addText("10 ACTIVE OPERATIONAL BASES",{x:rx,y:3.16,w:rw,h:0.26,fontFace:SANS_SB,fontSize:9,color:ORANGE,charSpacing:1.5,margin:0});
  ...
  let by=3.46; bases.forEach(([n,d])=>{ iconChip(s,rx,by+0.02,0.42,"mappin");
    ...
    s.addText(d,{x:rx+0.6,y:by+0.26,w:rw-0.6,h:0.36,fontFace:SANS,fontSize:8,color:GRAY,valign:"top",margin:0,lineSpacingMultiple:0.98}); by+=0.66; });
  ...
  sb.forEach((t,i)=>{ const x=M+i*sw2; if(i>0) s.addShape(p.ShapeType.line,{x,y:sy+0.05,w:0,h:0.55,line:{color:LINE,width:1}});
    s.addText(t[0],{x:x+0.2,y:sy,w:sw2-0.3,h:0.42,fontFace:SANS_SB,fontSize:20,color:ORANGE,margin:0,valign:"bottom"});
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  cards.forEach((c,i)=>{ const x=M+i*(cw+0.4), y=2.45;
    ...
    s.addText(c[1],{x:x+0.25,y:y+2.55,w:cw-0.5,h:0.4,fontFace:SANS_SB,fontSize:14,color:GRAPH,align:"center",valign:"top",margin:0});
    s.addText(c[2],{x:x+0.3,y:y+2.98,w:cw-0.6,h:1.1,fontFace:SANS,fontSize:9,color:GRAY,align:"center",valign:"top",margin:0,lineSpacingMultiple:1.05});
  });
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  team.forEach((t,i)=>{ const x=M+i*(gw+0.4), y=2.35, h=3.8;
    ...
    s.addText(t[1],{x:x+gw/2-d/2,y:y+0.35,w:d,h:d,fontFace:SANS_SB,fontSize:28,color:WHITE,align:"center",valign:"middle",margin:0});
    s.addText(t[0],{x:x+0.1,y:y+1.9,w:gw-0.2,h:0.35,fontFace:SANS_SB,fontSize:12,color:GRAPH,align:"center",margin:0});
    s.addText("[Role — TBC]",{x:x+0.1,y:y+2.25,w:gw-0.2,h:0.3,fontFace:SANS_M,fontSize:9,color:ORANGE,align:"center",margin:0});
    s.addShape(p.ShapeType.line,{x:x+0.4,y:y+2.68,w:gw-0.8,h:0,line:{color:LINE,width:1}});
    s.addText("[Background — to be\nconfirmed with the\nCompany]",{x:x+0.25,y:y+2.82,w:gw-0.5,h:0.9,fontFace:SANS,fontSize:8,color:GRAY,align:"center",valign:"top",margin:0,lineSpacingMultiple:1.1});
  });
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  s.addText("OPERATIONAL BRANCHES (FILIAIS)",{x:M,y:5.45,w:6,h:0.26,fontFace:SANS_SB,fontSize:9,color:ORANGE,charSpacing:1,margin:0});
  ...
  fil.forEach((f,i)=>{ const x=M+(i%fcols)*(fw+0.2), y=5.76+Math.floor(i/fcols)*0.5;
    rrect(s,x,y,fw,0.4,PAPER,{radius:0.05,line:LINE}); s.addText("Filial "+f,{x:x+0.05,y,w:fw-0.1,h:0.4,fontFace:SANS_M,fontSize:8,color:TXT,align:"center",valign:"middle",margin:0}); });
  rrect(s,W-M-2.5,2.17,2.5,0.62,GRAPH,{radius:0.08});
  s.addText([{text:"Fleet   ",options:{color:"AEB6C2",fontFace:SANS}},{text:"+400 vehicles",options:{color:ORANGE,fontFace:SANS_SB,fontSize:14}}],{x:W-M-2.5,y:2.17,w:2.5,h:0.62,fontSize:11,align:"center",valign:"middle",margin:0});
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  cases.forEach((c,i)=>{ const x=M+i*(cw+0.4), y=2.25, h=4.1;
    ...
    s.addText("[City — State]",{x:x+0.2,y:y+1.5,w:cw-0.4,h:0.3,fontFace:SANS_M,fontSize:9,color:GRAY,align:"center",margin:0});
    const sy=y+1.95, sw=(cw-0.6)/2;
    [["BRL [XXX] Mn","total value"],["[X] years","total term"]].forEach((t,j)=>{ const sx2=x+0.3+j*sw;
      s.addText(t[0],{x:sx2,y:sy,w:sw,h:0.4,fontFace:SANS_SB,fontSize:14,color:ORANGE,align:"center",valign:"middle",margin:0});
      s.addText(t[1],{x:sx2,y:sy+0.4,w:sw,h:0.26,fontFace:SANS,fontSize:8,color:GRAY,align:"center",valign:"top",margin:0}); });
    s.addShape(p.ShapeType.line,{x:x+0.4,y:sy+0.8,w:cw-0.8,h:0,line:{color:LINE,width:1}});
    s.addText("[Service scope — TBC]",{x:x+0.3,y:sy+0.95,w:cw-0.6,h:0.35,fontFace:SANS_M,fontSize:9,color:GRAPH,align:"center",margin:0});
    s.addText("[Relationship highlights and milestones to be confirmed with the Company]",{x:x+0.3,y:sy+1.3,w:cw-0.6,h:0.6,fontFace:SANS,fontSize:8,color:GRAY,align:"center",valign:"top",margin:0,lineSpacingMultiple:1.05});
  });
})();
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  tiles.forEach((t,i)=>{ const x=M+i*(tw+0.3), y=2.05, h=1.15;
    ...
    s.addText(t[1],{x:x+0.26,y:y+0.68,w:tw-0.44,h:0.32,fontFace:SANS,fontSize:9,color:GRAY,margin:0,valign:"top"}); });
...
(()=>{ const s=p.addSlide(); bg(s,WHITE);
  ...
  rows.forEach((r,ri)=>{ const kind=r[6];
    ...
    s.addText(r[0],{x:tx+0.2,y,w:colLabW-0.2,h:rh,fontFace:bold?SANS_SB:SANS,fontSize:9,italic:ital,color:ital?GRAY:TXT,valign:"middle",margin:0});
    for(let c=1;c<cols.length;c++){ const v=r[c]; const hot=(typeof v==="string"&&v.indexOf("~")>=0);
      s.addText(v,{x:tx+colLabW+(c-1)*dataW,y,w:dataW,h:rh,fontFace:bold?SANS_SB:SANS,fontSize:9,italic:ital,color:hot?ORANGE_D:(ital?GRAY:TXT),align:"center",valign:"middle",margin:0}); }
    y+=rh;
  });
})();
...
(()=>{ const s=p.addSlide(); bg(s,GRAPH);
  ...
  s.addText([{text:"Av. Brigadeiro Faria Lima, 2277 – 6th floor",options:{breakLine:true}},{text:"01452-000, Jardim Paulistano, São Paulo – SP",options:{breakLine:true}},{text:"+55 11 3815-3533",options:{}}],
    {x:cut+0.82,y:5.45,w:4.4,h:0.95,fontFace:SANS,fontSize:9,color:"AEB6C2",valign:"top",lineSpacingMultiple:1.2,margin:0});
